...
pub struct ClientState {
    ...
    /// Pending file downloads: file_id → save_path
    pub pending_downloads: RwLock<HashMap<String, String>>,
...
pub async fn connect_to_server(
    ...
    *state.pending_downloads.write().await = HashMap::new();
...
/// - **FileData**: Decrypt and save to disk
                ...
                match server_msg {
                    ...
                    ServerMessage::RoomJoined {
                        ...
                        *state.current_room.write().await = Some(room_id.clone());
                        ...
                        // (must read session_keys first, release, then write)
                        ...
                        let _ = app.emit(
                            ...
                            serde_json::json!({
                                ...
                                "config": {
                                    ...
                                    "file_expiry_secs": config.file_expiry_secs,
                                },
                    ...
                    ServerMessage::MemberKeys {
                        ...
                        // This sends them the current group key (post-ratchet)
                        // encrypted so only they can read it.
                        let payload_to_send = {
                            let keys = state.session_keys.read().await;
                    ...
                    // ── File Data (decrypt and save) ─────────────────────
                    ServerMessage::FileData { file_id, payload } => {
                        let save_path =
                            state.pending_downloads.write().await.remove(&file_id);

                        if let Some(path) = save_path {
                            let result = {
                                let ratchet = state.group_ratchet.read().await;
                                match &*ratchet {
                                    Some(r) => encryption::decrypt(&payload, r.current_key())
                                        .map_err(|e| format!("{}", e)),
                                    None => Err("No group key".to_string()),
                                }
                            };

                            let save_to = if path.is_empty() {
                                file_id.clone()
                            } else {
                                path
                            };
                            ...
                            match result {
                                Ok(data) => match tokio::fs::write(&save_to, &data).await {
                                    Ok(_) => {
                                        let _ = app.emit(
                                            "retro://file-downloaded",
                                            serde_json::json!({
                                                "file_id": file_id,
                                                "path": save_to,
                                                "size": data.len(),
...
/// Used by the receive loop for key exchange responses.
...
/// Returns an error if not connected or serialization fails.
