...
pub fn initiate_key_exchange(
    ...
    // 5. Ed25519 sign the entire payload for authentication
    let payload_json = serde_json::to_string(&encrypted_group_key)
        .map_err(|e| CryptoError::Serialization(e.to_string()))?;
    let sig_data = format!("{}{}", rsa_wrapped_b64, payload_json);
    let signature = our_keys.ed25519_signing.sign(sig_data.as_bytes());
...
pub fn complete_key_exchange(
    ...
    let payload_json = serde_json::to_string(&payload.encrypted_group_key)
        .map_err(|e| CryptoError::Serialization(e.to_string()))?;
    let sig_data = format!("{}{}", payload.rsa_wrapped_secret, payload_json);
    let sig_bytes = B64
    ...
    sender_verifying
        .verify(sig_data.as_bytes(), &signature)
        .map_err(|_| CryptoError::SignatureVerification)?;
...
pub fn derive_dm_key(
    ...
    // HKDF-SHA256 with DM-specific info for domain separation
